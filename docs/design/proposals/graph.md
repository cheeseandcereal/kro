...
- **`ResourcesConverged`**: Reports the executor's terminal apply and readiness state.
  ...
  - Status `False` with reason `StatusWriteFailed` when apply succeeded but the contribution inventory could not be persisted. The message identifies the persistence failure; the last durable inventory is retained. This condition is reported when the subsequent conditions write succeeds.
...
| `ResourcesConverged` | False   | `StatusWriteFailed`  | Applied, but contribution inventory persistence failed |
