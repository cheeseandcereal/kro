...
3. **Create the applier ServiceAccount and its RBAC.** The ServiceAccount named
   ...
   or deletes, plus `list` and `watch` on anything the Graph reads through a
   `ref` node.

4. **Grant users access to the `graphs` resource.** The chart deliberately does
