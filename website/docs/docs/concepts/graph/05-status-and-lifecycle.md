...
retries with backoff. `InvalidGraph` and `ApplyFailed` require a change to the
spec or the cluster.
...
| `internal.kro.run/apply-order` | annotation | The node's position in the dependency order. Deletion runs highest first. |
...
1. Every entry in `status.managedResources` is deleted, in reverse apply order.
...
When a Graph is waiting (`WaitingForReadiness` or `DataPending`), kro requeues
it with exponential backoff starting at one second and capped at five minutes.
