...
// nodeIDToken returns the bounded, label-safe rendering of a node's qualified
// path for the kro.run/node-id label value and the collection watch selector.
// The format lives in metadata.NodeIDToken, shared with the instance
// controller's prune attribution so writer and reader agree by construction.
